...
apple_mobility <- apple_mobility %>% select(- alternative_name, -geo_type, -alternative_name, -region,-sub.region)
apple_mobility <- apple_mobility %>% select(country,transportation_type,X2020.01.14:X2021.07.06)

apple_longer <- apple_mobility %>% pivot_longer(col = X2020.01.14:X2021.07.06)
short_list <- apple_longer %>% filter(country %in% c("Germany","Sweden"))
```
```{r}
 short_list$name <- gsub("X","",short_list$name)
 short_list$name <- gsub('\\.', '-', short_list$name)
 short_list$name <- as.Date(short_list$name)
...
```{r}
Sweden_Walking <- short_list %>% dplyr::filter(country == "Sweden",transportation_type == "walking")  
Sweden_transit <- short_list %>% dplyr::filter(country == "Sweden",transportation_type == "transit") 
Sweden_driving <- short_list %>% dplyr::filter(country == "Sweden",transportation_type == "driving")
...
ggplot() + geom_col(aes(x=name,y=value,fill=country)) +
  labs(title = "Driving") + scale_y_continuous(labels = comma)

short_list %>%
ggplot() + geom_col(aes(x=name,y=value,fill=transportation_type)) + 
  facet_wrap(~country)
```
...
group_by(country, Month = floor_date(name,"month")) %>%
  summarise(Total = sum(value),
            Mean = mean(value),
            Median = median(value),
            Percent = value/sum(value)) %>% ungroup()

```

```{r}
short_list %>% filter(transportation_type =="driving", country =="Germany") %>%
ggplot() + geom_point(aes(x=name,y=value,fill=country)) +
  labs(title = "Driving") + scale_y_continuous(labels = comma) +
  geom_smooth(data= short_list,aes(x=name,y=value),span=0.10)
```